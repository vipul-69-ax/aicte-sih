...
import { Search, Bell, FileText, Plus, Filter, ChevronDown, ChevronRight } from 'lucide-react'
...
const applicationTypes = [
  ...
  "Collaboration & Twinning Programs",
  "Open and Distance Learning (ODL)/Online Learning (OL)",
  "Penal Actions (For Norm Violations)",
  "Hibernation or Closure of Institutions",
]
...
const ApplicationsPage: React.FC = () => {
  ...
  const applyFilters = (search: string, statuses: string[], type: string) => {
    ...
    if (statuses.length > 0) {
      result = result.filter(app =>
        statuses.includes(getApplicationStatus(app.documents))
      )
    }

    if (type) {
      result = result.filter(app => app.application_type === type)
    }
    ...
    <div className="min-h-screen bg-[#f8f9fa]">
      <header className="bg-white border-b px-6 py-4">
        <div className="max-w-7xl mx-auto flex items-center justify-between">
          <div>
            <h1 className="text-2xl font-semibold text-[#2c3e50]">Applications Dashboard</h1>
            <p className="text-[#7f8c8d] text-sm">Manage and monitor your institution's applications</p>
          </div>
      ...
      <main className="max-w-7xl mx-auto p-6 space-y-6">
        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
          <Card className="bg-white shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-[#2c3e50]">Total Applications</CardTitle>
              <FileText className="h-4 w-4 text-[#3498db]" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-[#2c3e50]">{totalApplications}</div>
          ...
          <Card className="bg-white shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-[#2c3e50]">Approved Applications</CardTitle>
              <FileText className="h-4 w-4 text-[#2ecc71]" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-[#2c3e50]">{approvedApplications}</div>
              <p className="text-xs text-[#7f8c8d]">
                +15% from last month
              </p>
            </CardContent>
          ...
          <Card className="bg-white shadow-sm">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-2">
              <CardTitle className="text-sm font-medium text-[#2c3e50]">In Progress Applications</CardTitle>
              <FileText className="h-4 w-4 text-[#f39c12]" />
            </CardHeader>
            <CardContent>
              <div className="text-2xl font-bold text-[#2c3e50]">{inProgressApplications}</div>
              <p className="text-xs text-[#7f8c8d]">
                +5% from last month
              </p>
            </CardContent>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
          <Card className="bg-white shadow-sm">
            <CardHeader>
              <CardTitle className="text-[#2c3e50]">Application Submissions</CardTitle>
            </CardHeader>
            <CardContent>
          ...
          <Card className="bg-white shadow-sm">
            <CardHeader>
              <CardTitle className="text-[#2c3e50]">Recent Applications</CardTitle>
            </CardHeader>
            <CardContent>
              <ScrollArea className="h-[200px]">
                {applications.slice(applications.length-3, applications.length).reverse().map((app) => (
                  <div key={app.application_id} className="flex items-center space-x-4 mb-4">
                    <div className="flex-1 space-y-1">
                      <p className="text-sm font-medium leading-none text-[#2c3e50]">{app.application_name}</p>
                      <p className="text-sm text-[#7f8c8d]">{app.application_type}</p>
                    </div>
                    <Badge>
            ...
            <div className="flex items-center space-x-2">
              <Popover>
                <PopoverTrigger asChild>
                  <Button variant="outline" className="border-[#e0e0e0] text-[#7f8c8d]">
                ...
                <PopoverContent className="w-[220px] p-4">
                  <div className="space-y-4">
                    <div>
                      <h4 className="font-medium mb-2 text-[#2c3e50]">Status</h4>
                      <div className="space-y-2">
                        {["Approved", "In Progress", "Not Started"].map((status) => (
                          <div key={status} className="flex items-center">
                            ...
                            />
                            <Label htmlFor={`status-${status}`} className="ml-2 text-sm text-[#2c3e50]">
                              {status}
                            </Label>
                          </div>
                        ))}
              ...
              <Button onClick={() => navigate("/institute")} className="bg-[#3498db] hover:bg-[#2980b9] text-white">
          ...
          <CardContent>
            <div className="space-y-4 max-h-[calc(100vh-24rem)] overflow-auto">
              {filteredApplications.map((app) => (
                <div key={app.application_id} className="flex items-center justify-between p-4 bg-white rounded-lg border border-[#e0e0e0]">
                  <div className="flex items-center space-x-4">
                    <Avatar>
                      <AvatarFallback>{app.application_name.charAt(0)}</AvatarFallback>
                    </Avatar>
                    <div>
                      <p className="text-sm font-medium text-[#2c3e50]">{app.application_name}</p>
                      <p className="text-sm text-[#7f8c8d]">{app.application_type}</p>
                    </div>
                  </div>
                  <div className="flex items-center space-x-4">
                    ...
                    <Button variant="ghost" size="icon" onClick={() => navigate(`/institute/applications/${app.application_id}
`)}>
