...
export default function UploadLegalDocument() {
  ...
  const {mutate:verifyFile, isPending:isVerifying, data:errorData} = useFileVerification()
  ...
  return (
    <div className="min-h-screen bg-gray-100 py-12 px-4 sm:px-6 lg:px-8" style={{ backgroundColor: aicteTheme.background }}>
      <div className="max-w-4xl mx-auto">
        <div className="text-center mb-8">
          <img src={AicteLogo} alt="AICTE Logo" className="mx-auto w-24 h-24 mb-4" />
          <h1 className="text-3xl font-bold" style={{ color: aicteTheme.primary }}>AICTE Document Upload Portal</h1>
        </div>
        <Card className="w-full shadow-lg">
          <CardHeader style={{ backgroundColor: aicteTheme.primary, color: 'white' }}>
            <CardTitle className="text-2xl font-bold">Upload {doc.uni_doc_name}</CardTitle>
          </CardHeader>
          <CardContent className="p-6">
            {!file && (
              <div className="flex items-center justify-center w-full">
                <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-dashed rounded-lg cursor-pointer hover:bg-gray-50 transition-colors duration-300" style={{ borderColor: aicteTheme.primary }}>
                  <div className="flex flex-col items-center justify-center pt-5 pb-6">
                    <Upload className="w-12 h-12 mb-3" style={{ color: aicteTheme.primary }} />
                    <p className="mb-2 text-sm"><span className="font-semibold">Click to upload</span> or drag and drop</p>
                    <p className="text-xs" style={{ color: aicteTheme.text }}>PDF (MAX. 10MB)</p>
                  </div>
            ...
            {pdfUrl && (
              <div className="mt-4">
                <div className="flex items-center mb-2">
                  <FileText className="mr-2" style={{ color: aicteTheme.primary }} />
            ...
            {isSuccess && data && (
              <Alert className="mt-4" style={{ backgroundColor: aicteTheme.accent, color: 'white' }}>
          ...
          <CardFooter className="flex justify-between bg-gray-50">
            <div>
              {file && !isSuccess && (
                <Button onClick={handleUpload} disabled={isPending} style={{ backgroundColor: aicteTheme.primary }}>
...
function ForgeryGuidelines() {
  return (
    <Card className="mt-6 shadow-lg">
      <CardHeader style={{ backgroundColor: aicteTheme.secondary, color: 'white' }}>
